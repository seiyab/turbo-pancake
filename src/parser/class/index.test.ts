...
import { bodyParser, classParser, fieldParser, relationParser } from ".";
...
describe("classParser", () => {
  ...
  it("class body", () => {
    const input = ["class Foo {", "  int id", "  String name", "}"].join("\n");
    const { value: result } = classParser().parse(Streams.ofString(input));
    expect(result.type).toEqual("class");
    expect(result.value).toEqual({
      name: "Foo",
  ...
  it("stereotype is not implemented yet", () => {
    ...
    const { value: result } = classParser()
      .rep()
      .parse(Streams.ofString(input));
    expect(result.value).toEqual([
      {
        type: "class",
        value: { name: "Class01", body: null },
      },
      { type: "class", value: { name: "Class02", body: null } },
    ]);
  });
});
...
describe("bodyParser", () => {
  ...
  it("multiple fields", () => {
    const input = ["{", "  int foo", "  String bar", "  boolean baz", "}"].join(
      "\n"
    );
    const result = bodyParser().val(input);
    expect(result).toEqual({
...
describe("relationParser", () => {
  it("o--", () => {
    const input = "foo o-- bar";
    const result = relationParser().val(input);
    expect(result).toEqual({
      type: "relation",
      value: {
        left: {
